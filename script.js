(() => {
  ...
  const elRankedTrialBtn = /** @type {HTMLButtonElement | null} */ ($("rankedTrialBtn"));
  const elRankedTrialStatus = $("rankedTrialStatus");
  const elStreakCoinPill = $("streakCoinPill");
  const elGlobalLevelsSortSelect = /** @type {HTMLSelectElement | null} */ ($("globalLevelsSortSelect"));
  const elGlobalLevelsAutoFeatured = $("globalLevelsAutoFeatured");
  const elTitlePickerList = $("titlePickerList");
  const elRingPickerList = $("ringPickerList");
  ...
  /** @type {{ id: string, label: string, achievementId: string }[]} */
  const TITLE_DEFS = [
    { id: "title_first_win", label: "First Victory", achievementId: "ach_first_win" },
    { id: "title_streak_5", label: "Hot Streak", achievementId: "ach_win_streak_5" },
    { id: "title_builtin_10", label: "Catalog Runner", achievementId: "ach_builtin_wins_10" },
    { id: "title_daily", label: "Daily Grinder", achievementId: "ach_daily_once" },
    { id: "title_ranked", label: "Trial Champion", achievementId: "ach_ranked_once" },
    { id: "title_hard_5", label: "Brass Knuckles", achievementId: "ach_hard_wins_5" },
    { id: "title_points_5k", label: "Point Hoarder", achievementId: "ach_points_5000" },
    { id: "title_runs_50", label: "Persistent", achievementId: "ach_runs_50" },
    { id: "title_publisher", label: "Published", achievementId: "ach_publish_global" },
    { id: "title_collector", label: "Skin Collector", achievementId: "ach_own_3_skins" },
  ];

  /** @type {{ id: string, label: string, achievementId: string }[]} */
  const RING_DEFS = [
    { id: "default", label: "Default blue", achievementId: "" },
    { id: "gold", label: "Gold ring", achievementId: "ach_points_5000" },
    { id: "ember", label: "Ember ring", achievementId: "ach_hard_wins_5" },
    { id: "mint", label: "Mint ring", achievementId: "ach_ranked_once" },
    { id: "void", label: "Void ring", achievementId: "ach_win_streak_5" },
  ];

  /** @type {{ id: string, name: string, desc: string, test: (ctx: { p: PlayerRecord, sabotageMeta: typeof sabotageMeta }) => boolean }[]} */
  const ACHIEVEMENT_DEFS = [
    { id: "ach_first_win", name: "First win", desc: "Win any level once.", test: (c) => (c.p.stats.totalWins | 0) >= 1 },
    { id: "ach_runs_10", name: "Getting warm", desc: "Complete 10 runs (win or lose).", test: (c) => (c.p.stats.totalRuns | 0) >= 10 },
    { id: "ach_runs_50", name: "Marathon mind", desc: "Complete 50 runs.", test: (c) => (c.p.stats.totalRuns | 0) >= 50 },
    { id: "ach_win_streak_3", name: "Triple threat", desc: "3-win streak.", test: (c) => (c.p.stats.winStreak | 0) >= 3 },
    { id: "ach_win_streak_5", name: "On fire", desc: "5-win streak.", test: (c) => (c.p.stats.winStreak | 0) >= 5 },
    { id: "ach_win_streak_10", name: "Unstoppable", desc: "10-win streak.", test: (c) => (c.p.stats.winStreak | 0) >= 10 },
    { id: "ach_deaths_20", name: "Rubber bones", desc: "20 total deaths (learning is winning).", test: (c) => (c.p.stats.totalDeaths | 0) >= 20 },
    { id: "ach_builtin_wins_5", name: "Tourist", desc: "5 total wins.", test: (c) => (c.p.stats.totalWins | 0) >= 5 },
    { id: "ach_builtin_wins_10", name: "Speed tourist", desc: "10 total wins.", test: (c) => (c.p.stats.totalWins | 0) >= 10 },
    { id: "ach_hard_wins_3", name: "Spikes were friends", desc: "3 hard-tier built-in wins tracked.", test: (c) => ((c.p.stats.hardBuiltinWins | 0)) >= 3 },
    { id: "ach_hard_wins_5", name: "Hard mode regular", desc: "5 hard-tier built-in wins tracked.", test: (c) => ((c.p.stats.hardBuiltinWins | 0)) >= 5 },
    { id: "ach_points_1000", name: "Scorer", desc: "Earn 1000 total points.", test: (c) => (c.p.stats.totalPointsEarned | 0) >= 1000 },
    { id: "ach_points_5000", name: "Big scorer", desc: "Earn 5000 total points.", test: (c) => (c.p.stats.totalPointsEarned | 0) >= 5000 },
    { id: "ach_saved_levels_3", name: "Architect", desc: "Save 3 custom levels.", test: (c) => Object.keys(c.p.levels || {}).length >= 3 },
    { id: "ach_powerup_hoard", name: "Prepared", desc: "Hold 6+ powerups combined.", test: (c) => totalPowerups(c.p) >= 6 },
    { id: "ach_coins_local_500", name: "Piggy bank", desc: "Reach 500 local coins.", test: (c) => (c.p.localCoins | 0) >= 500 },
    { id: "ach_almost", name: "So close", desc: "10 almost-wins (near goal).", test: (c) => (c.sabotageMeta.almostWins | 0) >= 10 },
    { id: "ach_daily_once", name: "Daily duelist", desc: "Clear a daily challenge (tracked locally).", test: (c) => !!(c.p.stats && (c.p.stats.dailyClears | 0) >= 1) },
    { id: "ach_ranked_once", name: "Ranked graduate", desc: "Finish a ranked trial day.", test: (c) => ((c.p.stats.rankedTrialsCompleted | 0) >= 1) },
    { id: "ach_publish_global", name: "Cloud builder", desc: "Publish a level to global while online.", test: (c) => !!(c.p.stats && (c.p.stats.globalPublishCount | 0) >= 1) },
    { id: "ach_own_3_skins", name: "Wardrobe", desc: "Own 3 shop skins.", test: (c) => (c.p.ownedAvatarIds && c.p.ownedAvatarIds.length) >= 3 },
    { id: "ach_tutorial_all", name: "Schooled", desc: "Beat all 5 tutorial maps.", test: (c) => tutorialAllDone(c.p) },
    { id: "ach_medium_wins_8", name: "Middle child", desc: "8 medium-tier built-in wins tracked.", test: (c) => ((c.p.stats.mediumBuiltinWins | 0) >= 8) },
    { id: "ach_legend_combo", name: "Legend combo", desc: "10 streak + 5k points.", test: (c) => (c.p.stats.winStreak | 0) >= 10 && (c.p.stats.totalPointsEarned | 0) >= 5000 },
    { id: "ach_grinder", name: "Grinder", desc: "100 total runs.", test: (c) => (c.p.stats.totalRuns | 0) >= 100 },
    { id: "ach_coin_spender", name: "Treat yourself", desc: "Buy any shop skin.", test: (c) => (c.p.ownedAvatarIds && c.p.ownedAvatarIds.length) >= 1 },
    { id: "ach_hex_veteran", name: "Hex aware", desc: "20 total wins.", test: (c) => (c.p.stats.totalWins | 0) >= 20 },
    { id: "ach_best_diff_80", name: "Climber", desc: "Beat difficulty score 80+ on a win.", test: (c) => (c.p.stats.bestDifficultyBeaten | 0) >= 80 },
    { id: "ach_local_mp", name: "Couch chaos", desc: "Play with local multiplayer on.", test: (c) => !!(c.p.stats && (c.p.stats.localMpRounds | 0) >= 1) },
  ];

  function totalPowerups(p) {
    const pw = p && p.powerups ? p.powerups : {};
    return (pw.doubleJump | 0) + (pw.speedBoost | 0) + (pw.protection | 0);
  }

  function tutorialAllDone(p) {
    const ids = new Set(["builtin_tut_spikes", "builtin_tut_pad", "builtin_tut_hex", "builtin_tut_lava", "builtin_tut_platform"]);
    const done = new Set(Array.isArray(p.completedBuiltinLevelIdsForCoins) ? p.completedBuiltinLevelIdsForCoins : []);
    for (const id of ids) if (!done.has(id)) return false;
    return true;
  }

  function evaluateAchievementsForActivePlayer() {
    if (!activePlayer) return;
    const unlocked = new Set(Array.isArray(activePlayer.unlockedAchievementIds) ? activePlayer.unlockedAchievementIds : []);
    const ctx = { p: activePlayer, sabotageMeta };
    let anyNew = false;
    for (const def of ACHIEVEMENT_DEFS) {
      ...
      try {
        if (def.test(ctx)) {
          unlocked.add(def.id);
          anyNew = true;
          showToast(`Achievement: ${def.name}`, 2400);
        }
      } catch {
        /* ignore */
      }
    }
    if (anyNew) activePlayer.unlockedAchievementIds = Array.from(unlocked);
  }

  function hasAchievement(id) {
    if (!activePlayer || !Array.isArray(activePlayer.unlockedAchievementIds)) return false;
    return activePlayer.unlockedAchievementIds.includes(id);
  }

  function renderTitleRingAchievementPickers() {
    if (elTitlePickerList) {
      elTitlePickerList.innerHTML = "";
      const none = document.createElement("button");
      none.type = "button";
      none.className = "btn subtle";
      none.textContent = !activePlayer || !activePlayer.equippedTitleId ? "No title (equipped)" : "Clear title";
      none.addEventListener("click", () => {
        if (!activePlayer) return;
        activePlayer.equippedTitleId = null;
        persist();
        syncProfileUI();
        renderTitleRingAchievementPickers();
      });
      elTitlePickerList.appendChild(none);
      for (const t of TITLE_DEFS) {
        const ok = !t.achievementId || hasAchievement(t.achievementId);
        ...
        name.className = "name";
        name.textContent = t.label + (ok ? "" : " (locked)");
        meta.appendChild(name);
        const btn = document.createElement("button");
        btn.type = "button";
        btn.className = "btn primary";
        btn.disabled = !ok || (activePlayer && activePlayer.equippedTitleId === t.id);
        btn.textContent = activePlayer && activePlayer.equippedTitleId === t.id ? "Equipped" : "Equip";
        btn.addEventListener("click", () => {
          if (!activePlayer || !ok) return;
          activePlayer.equippedTitleId = t.id;
          persist();
          syncProfileUI();
          renderTitleRingAchievementPickers();
        });
        ...
        row.appendChild(btn);
        elTitlePickerList.appendChild(row);
      }
    }
    if (elRingPickerList) {
      elRingPickerList.innerHTML = "";
      for (const r of RING_DEFS) {
        const ok = !r.achievementId || hasAchievement(r.achievementId);
        const row = document.createElement("div");
        row.className = "listItem";
        const meta = document.createElement("div");
        meta.className = "meta";
        const name = document.createElement("div");
        name.className = "name";
        name.textContent = r.label + (ok ? "" : " (locked)");
        meta.appendChild(name);
        const btn = document.createElement("button");
        btn.type = "button";
        btn.className = "btn primary";
        const cur = activePlayer && activePlayer.profileRingStyle ? activePlayer.profileRingStyle : "default";
        btn.disabled = !ok || cur === r.id;
        btn.textContent = cur === r.id ? "Equipped" : "Equip";
        btn.addEventListener("click", () => {
          if (!activePlayer || !ok) return;
          activePlayer.profileRingStyle = r.id;
          persist();
        ...
        row.appendChild(meta);
        row.appendChild(btn);
        elRingPickerList.appendChild(row);
      }
    }
    if (elAchievementList) {
      elAchievementList.innerHTML = "";
      if (!activePlayer) {
        elAchievementList.appendChild(makeEmptyLine("Choose a player to track achievements."));
        return;
      }
      for (const def of ACHIEVEMENT_DEFS) {
        const got = hasAchievement(def.id);
        const row = document.createElement("div");
        row.className = "listItem";
        const meta = document.createElement("div");
        meta.className = "meta";
        ...
        row.appendChild(meta);
        elAchievementList.appendChild(row);
      }
    }
  }
  ...
  function applyCosmeticToProfileChip() {
    ...
    const ring = activePlayer && activePlayer.profileRingStyle ? String(activePlayer.profileRingStyle) : "default";
    const ringShadow =
      ring === "gold"
        ? "0 0 0 3px rgba(234,179,8,0.55), 0 0 14px rgba(250,204,21,0.35)"
        : ring === "ember"
          ? "0 0 0 3px rgba(249,115,22,0.55), 0 0 14px rgba(251,146,60,0.35)"
          : ring === "mint"
            ? "0 0 0 3px rgba(52,211,153,0.55), 0 0 14px rgba(16,185,129,0.3)"
            : ring === "void"
              ? "0 0 0 3px rgba(99,102,241,0.55), 0 0 16px rgba(129,140,248,0.45)"
              : "0 0 0 3px rgba(122, 167, 255, 0.18)";
    if (!equippedAvatar) {
      dot.style.background =
        "radial-gradient(circle at 30% 30%, rgba(255, 255, 255, 0.92), rgba(122, 167, 255, 0.55))";
      dot.style.boxShadow = ringShadow;
      return;
    }
    dot.style.background =
    ...
    dot.style.boxShadow =
      ring === "default"
        ? "0 0 0 3px rgba(34, 197, 94, 0.45), 0 0 14px rgba(34, 197, 94, 0.25)"
        : ringShadow;
  }
   ...
   */
  async function pushGlobalScore(earnedRunPoints, preconfiguredMeta, streakForCoinMult = 0) {
        ...
        try {
          ...
          if (coinErr) logSupabaseError("ssb_award_preconfigured_coins", coinErr, { user_id: user.id, level_id: lid });
            ...
            if (Number.isFinite(n) && n > 0) {
              userCoins += n;
              const sm = typeof streakForCoinMult === "number" ? winStreakCoinMultiplier(streakForCoinMult) : 1;
              const extra = Math.max(0, Math.min(120, Math.floor(n * (sm - 1))));
              if (extra > 0) {
                try {
                  const { data: ex, error: exErr } = await sb.rpc("ssb_grant_streak_bonus_coins", { p_extra: extra });
                  if (exErr) logSupabaseError("ssb_grant_streak_bonus_coins", exErr, { p_extra: extra });
                  else if (typeof ex === "number" && ex > 0) {
                    userCoins += ex;
                    showToast(`Streak bonus: +${ex} coins`, 2000);
                  }
                } catch (e) {
                  logSupabaseError("ssb_grant_streak_bonus_coins (exception)", e, {});
                }
              }
            }
          }
  ...
  const MUSIC_LIBRARY = /** @type {{ id: string, label: string, file: string }[]} */ ([
    ...
    { id: "t1", label: "Soundtrack 1", file: "Death Note Opening 1 - The World_default.mp3" },
    { id: "t2", label: "Soundtrack 2", file: "Dr. Stone Opening 2 - Sangenshoku_tv.mp3" },
    { id: "t3", label: "Soundtrack 3", file: "Shingeki no Kyojin Opening 1 - Feuerroter Pfeil und Bogen_default.mp3" },
    { id: "t4", label: "Soundtrack 4", file: "Vinland Saga Opening 1 - MUKANJYO_tv.mp3" },
  ]);
  ...
  /** Top community levels by engagement (likes − dislikes), for Levels modal rotation. */
  let cachedTrendingGlobalLevels = /** @type {any[]} */ ([]);
  /** @type {"new"|"trending"|"liked"} */
  let globalLevelsSortMode = "new";
   ...
   * @property {number} [hardBuiltinWins]
   * @property {number} [mediumBuiltinWins]
   * @property {number} [dailyClears]
   * @property {number} [rankedTrialsCompleted]
   * @property {number} [globalPublishCount]
   * @property {number} [localMpRounds]
   */
  ...
  * @property {string|null} [equippedAvatarId]
   ...
   * @property {string|null} [equippedTitleId]
   * @property {string} [profileRingStyle]
   * @property {string[]} [cloudPublishedLevelIds]
   */
  ...
  function loadSave() {
    try {
      ...
      for (const p of Object.values(parsed.players)) {
        ...
        if (p.stats && typeof p.stats.hardBuiltinWins !== "number") p.stats.hardBuiltinWins = 0;
        if (p.stats && typeof p.stats.mediumBuiltinWins !== "number") p.stats.mediumBuiltinWins = 0;
        if (p.stats && typeof p.stats.dailyClears !== "number") p.stats.dailyClears = 0;
        if (p.stats && typeof p.stats.rankedTrialsCompleted !== "number") p.stats.rankedTrialsCompleted = 0;
        if (p.stats && typeof p.stats.globalPublishCount !== "number") p.stats.globalPublishCount = 0;
        if (p.stats && typeof p.stats.localMpRounds !== "number") p.stats.localMpRounds = 0;
        ...
        if (!Array.isArray(p.unlockedAchievementIds)) p.unlockedAchievementIds = [];
        if (typeof p.equippedTitleId !== "string") p.equippedTitleId = null;
        else if (!String(p.equippedTitleId).trim()) p.equippedTitleId = null;
        if (typeof p.profileRingStyle !== "string" || !p.profileRingStyle.trim()) p.profileRingStyle = "default";
        if (!Array.isArray(p.cloudPublishedLevelIds)) p.cloudPublishedLevelIds = [];
      }
  ...
  function createPlayer(name) {
    ...
    const p = /** @type {PlayerRecord} */ ({
      ...
      profileRingStyle: "default",
      cloudPublishedLevelIds: [],
    });
    save.players[id] = p;
  ...
  function syncProfileUI() {
    ...
    if (sub) {
      if (online) {
        ...
        const td = TITLE_DEFS.find((t) => t.id === activePlayer.equippedTitleId);
        const parts = [];
        if (td) parts.push(td.label);
        if (streak > 0) parts.push(`${streak} win streak`);
        if (parts.length) {
  ...
  /** Looped MP3 soundtrack (separate from WebAudio SFX / ambient). */
  const MusicSys = (() => {
    const el = new Audio();
    el.preload = "auto";
    let lastSrc = "";
    ...
    function apply() {
      const id = save.settings.musicTrack || "off";
      const tr = trackById(id);
      const vol = Math.max(0, Math.min(1, save.settings.volume ?? 0.7)) * 0.34;
      if (!save.settings.sound || tr.id === "off" || !tr.file) {
        el.pause();
      ...
      el.volume = vol;
      el.play().catch(() => {
        /* autoplay policy — user gesture unlocks via pointerdown */
      });
    }
    return { apply };
  })();
  ...
  const RANKED_TRIAL_LOCK_SABOTAGE = 7;
  const STREAK_COIN_RATE = 0.04;
  const STREAK_COIN_CAP_MULT = 1.5;

  function winStreakCoinMultiplier(prevStreakBeforeThisWin) {
    const s = Math.max(0, Math.floor(Number(prevStreakBeforeThisWin) || 0));
    const maxSteps = Math.max(0, Math.ceil((STREAK_COIN_CAP_MULT - 1) / STREAK_COIN_RATE));
    return Math.min(STREAK_COIN_CAP_MULT, 1 + Math.min(s, maxSteps) * STREAK_COIN_RATE);
  }

  function rankedBuiltinReachable(lvl) {
    const g = makeGrid(COLS, ROWS, Tile.empty);
    inflateGrid(g, lvl.tilesFlat, COLS, ROWS);
    let pc = 0;
    let td = 0;
    for (let y = 0; y < ROWS; y++) {
      for (let x = 0; x < COLS; x++) {
        if (g[y][x] === Tile.pressureSwitch) pc++;
        if (g[y][x] === Tile.timedDoor) td++;
      }
    }
    if (pc + td > 0) return true;
    return runReachabilityBfs(g, isStandableReach, 1).reachesGoal;
  }

  function getRankedTrialPack(dayKey) {
    const easy = BUILTIN_LEVELS.filter((l) => l.tier === "easy");
    const med = BUILTIN_LEVELS.filter((l) => l.tier === "medium");
    const hard = BUILTIN_LEVELS.filter((l) => l.tier === "hard");
    const pick = (arr, salt) => arr[stableHashU32(String(dayKey) + salt) % arr.length];
    let e = pick(easy, "rtE");
    let m = pick(med, "rtM");
    let h = pick(hard, "rtH");
    const fix = (tier, cur) => {
      const arr = tier === "easy" ? easy : tier === "medium" ? med : hard;
      if (rankedBuiltinReachable(cur)) return cur;
      for (const x of arr) if (rankedBuiltinReachable(x)) return x;
      return cur;
    };
    e = fix("easy", e);
    m = fix("medium", m);
    ...
    return {
      ...
      sabotage: RANKED_TRIAL_LOCK_SABOTAGE,
      levels: [
        { id: e.id, builtinIndex: BUILTIN_LEVELS.findIndex((x) => x.id === e.id) },
        { id: m.id, builtinIndex: BUILTIN_LEVELS.findIndex((x) => x.id === m.id) },
        { id: h.id, builtinIndex: BUILTIN_LEVELS.findIndex((x) => x.id === h.id) },
      ],
    };
  }

  function computeRankedTrialScore(agg) {
    const t = Math.max(1, (agg.timeMs || 0) / 1000);
    const d = Math.max(0, agg.deaths | 0);
    const a = Math.max(1, agg.attempts | 0);
    const sab = Math.max(1, Math.min(10, agg.sabotage | 0));
    return Math.round(1_000_000 / t - d * 800 - (a - 3) * 350 + (11 - sab) * 140);
  }

  function syncRankedTrialStatusUI() {
    if (!elRankedTrialStatus) return;
    const dk = getUtcDayKey();
    let best = null;
    try {
      const raw = localStorage.getItem("ssb_ranked_trial_best_v1");
      if (raw) {
        const o = JSON.parse(raw);
        if (o && typeof o === "object" && o[dk] != null) best = Number(o[dk]);
      }
    } catch {
      /* ignore */
    }
    elRankedTrialStatus.textContent =
      best != null && Number.isFinite(best) ? `Best today: ${Math.round(best)}` : "No score yet today";
  }

  function syncStreakCoinPill() {
    if (!elStreakCoinPill || !activePlayer) return;
    const s = typeof activePlayer.stats.winStreak === "number" ? activePlayer.stats.winStreak : 0;
    const m = winStreakCoinMultiplier(s);
    elStreakCoinPill.textContent = `Win streak: ${s} · next coin bonus ×${m.toFixed(2)} (capped)`;
  }

  function startRankedTrial() {
    if (!activePlayer) {
      openStartModal();
      showToast("Choose a player first.", 2000);
      return;
    }
    const dayKey = getUtcDayKey();
    const pack = getRankedTrialPack(dayKey);
    const first = pack.levels[0];
    const bl = BUILTIN_LEVELS[first.builtinIndex];
    if (!bl) return;
    const session = {
      dayKey: pack.dayKey,
      sabotage: pack.sabotage,
      levels: pack.levels,
      step: 0,
      aggDeaths: 0,
      aggTimeMs: 0,
    ...
    startPlay(bl.id, first.builtinIndex, null, null, { rankedForcedSabotage: pack.sabotage, rankedTrialSession: session });
  }
  ...
  if (elExitToMenuBtn) {
    elExitToMenuBtn.addEventListener("click", () => {
      if (mode === "play" && play && !play.ended && activePlayer) {
        activePlayer.stats.winStreak = 0;
        if (play.rankedTrialSession) play.rankedTrialSession = null;
        persist();
        syncProfileUI();
        showToast("Menu: win streak reset (quit mid-run).", 2200);
      }
      if (mode === "play") {
        mode = "build";
        play = null;
    ...
    draftSaveTimer = window.setTimeout(() => {
      ...
      try {
        localStorage.setItem(
          DRAFT_GRID_KEY,
          JSON.stringify({ v: 2, t: Date.now(), flat: flattenGrid(grid), texts: cloneLevelTexts(), links: cloneLevelLinks() })
        );
      } catch {
        /* quota */
      }
  ...
  function importGridFromJsonText(text, silent = false) {
    ...
    if (!o || (o.v !== 1 && o.v !== 2) || !Array.isArray(o.tilesFlat)) {
      if (!silent) showToast("Invalid level JSON.");
      return;
    }
  ...
  async function refreshFeaturedGlobalLevelsCache() {
    ...
    try {
      ...
      const sel =
        "id, client_level_id, name, tiles_flat, texts, cols, rows, difficulty, like_count, dislike_count, list_in_global, created_at";
      const { data: tr, error: trErr } = await sb.from("global_levels").select(sel).limit(80);
      if (trErr) {
        logSupabaseError("global_levels.select(trending)", trErr, {});
        cachedTrendingGlobalLevels = [];
      } else {
        const rows = Array.isArray(tr) ? tr.filter((r) => r && r.list_in_global !== false) : [];
        rows.sort((a, b) => {
          const la = (a.like_count | 0) - (a.dislike_count | 0);
          const lb = (b.like_count | 0) - (b.dislike_count | 0);
          if (lb !== la) return lb - la;
          const ta = new Date(a.created_at || 0).getTime();
          const tb = new Date(b.created_at || 0).getTime();
          return tb - ta;
        });
        cachedTrendingGlobalLevels = rows.slice(0, 12);
      }
    } catch (e) {
  ...
  function renderTrendingFeaturedButtons() {
    if (!elFeaturedLevelsHost) return;
    const old = elFeaturedLevelsHost.querySelector("[data-trending-featured]");
    if (old) old.remove();
    const wrap = document.createElement("div");
    wrap.dataset.trendingFeatured = "1";
    wrap.style.marginTop = "10px";
    const lab = document.createElement("div");
    lab.className = "smallNote";
    lab.textContent = "Trending community (auto picks — sign in to publish)";
    ...
    const picks = (cachedTrendingGlobalLevels || []).slice(0, 3);
    if (picks.length === 0) {
      const p = document.createElement("div");
      p.className = "smallNote";
      p.textContent = "No trending data yet (likes need the optional SQL migration).";
      row.appendChild(p);
    } else {
      for (const gl of picks) {
        const btn = document.createElement("button");
        btn.type = "button";
        btn.className = "btn subtle";
        const likes = gl.like_count | 0;
        const dis = gl.dislike_count | 0;
        btn.textContent = `↗ ${String(gl.name || "Level")} (${likes}↑ ${dis}↓)`;
        btn.addEventListener("click", () => {
    ...
    elFeaturedLevelsHost.appendChild(wrap);
  }
  ...
  if (elGlobalLevelsSortSelect) elGlobalLevelsSortSelect.addEventListener("change", () => void refreshGlobalLevelsList(true));
  if (elRankedTrialBtn) elRankedTrialBtn.addEventListener("click", () => startRankedTrial());
    ...
    if (sb && u && u.id && isSupabaseConfigured() && !skipCloudUpload) {
      void (async () => {
        try {
          ...
          if (upErr) {
            logSupabaseError("global_levels.upsert (saveLevel)", upErr, { author_id: u.id, client_level_id: level.id });
          } else if (activePlayer) {
            const lid = String(level.id);
            const pubSet = new Set(Array.isArray(activePlayer.cloudPublishedLevelIds) ? activePlayer.cloudPublishedLevelIds : []);
            if (!pubSet.has(lid)) {
              pubSet.add(lid);
              activePlayer.cloudPublishedLevelIds = Array.from(pubSet);
              activePlayer.stats.globalPublishCount = (activePlayer.stats.globalPublishCount | 0) + 1;
              persist();
              evaluateAchievementsForActivePlayer();
            }
          }
          if (
            isAdminUser &&
  ...
  function refreshFeaturedLevelsUI() {
    ...
    void refreshFeaturedGlobalLevelsCache().then(() => renderTrendingFeaturedButtons());
  }
  ...
  async function voteGlobalLevel(gl, vote) {
    const sb = getSupabaseClient();
    const u = currentSupabaseUser;
    if (!sb || !u || !u.id || !isSupabaseConfigured()) {
      showToast("Sign in to vote on levels.", 2000);
      return;
    }
    if (!gl || !gl.id) return;
    try {
      const { error } = await sb.rpc("ssb_vote_global_level", { p_level_id: gl.id, p_vote: vote });
      if (error) {
        logSupabaseError("ssb_vote_global_level", error, { level_id: gl.id, vote });
        showToast("Vote failed — run optional SQL migration (enrichment_social_v1.sql).", 3200);
        return;
      }
      showToast(vote > 0 ? "Thanks — counted as like." : "Feedback saved.", 1600);
      void refreshGlobalLevelsList(true);
    } catch (e) {
      logSupabaseError("voteGlobalLevel (exception)", e, {});
    }
  }

  async function promptAndSaveGlobalTags(gl) {
    const sb = getSupabaseClient();
    const u = currentSupabaseUser;
    if (!sb || !u || !gl || !gl.id) return;
    if (gl.author_id && gl.author_id !== u.id) {
      showToast("Only the author can edit tags.", 2000);
      return;
    }
    const cur = Array.isArray(gl.tags) ? gl.tags.join(", ") : "";
    const raw = window.prompt("Comma-separated tags: speedrun, puzzle, chaos, beginner, hardcore", cur);
    if (raw == null) return;
    const arr = String(raw)
      ...
      .filter(Boolean);
    try {
      const { error } = await sb.rpc("ssb_set_global_level_tags", { p_level_id: gl.id, p_tags: arr });
      if (error) {
        logSupabaseError("ssb_set_global_level_tags", error, { level_id: gl.id });
        showToast("Could not save tags (migration installed?).", 2600);
        return;
      }
      showToast("Tags updated.", 1600);
      void refreshGlobalLevelsList(true);
    } catch (e) {
      logSupabaseError("promptAndSaveGlobalTags (exception)", e, {});
    }
  }
  ...
  async function refreshGlobalLevelsList(force = false) {
    ...
    if (elGlobalLevelsSortSelect) globalLevelsSortMode = /** @type {"new"|"trending"|"liked"} */ (elGlobalLevelsSortSelect.value || "new");

    elGlobalLevelsList.innerHTML = "";
    elGlobalLevelsList.appendChild(makeEmptyLine(force ? "Refreshing…" : "Loading…"));
    if (elGlobalLevelsAutoFeatured) elGlobalLevelsAutoFeatured.textContent = "";

    try {
      const { data, error } = await sb
        .from("global_levels")
        .select(
          "id, author_id, client_level_id, name, tiles_flat, texts, cols, rows, difficulty, validation_state, validation_notes, created_at, like_count, dislike_count, tags, profiles(username)"
        )
        .limit(80);
      if (error) {
      ...
      elGlobalLevelsList.innerHTML = "";
      let rows = Array.isArray(data) ? data.filter((r) => r && r.list_in_global !== false) : [];
      if (globalLevelsSortMode === "trending") {
        rows = [...rows].sort((a, b) => {
      ...
      } else if (globalLevelsSortMode === "liked") {
        rows = [...rows].sort((a, b) => (b.like_count | 0) - (a.like_count | 0));
      } else {
        rows = [...rows].sort((a, b) => new Date(b.created_at || 0).getTime() - new Date(a.created_at || 0).getTime());
      }
      rows = rows.slice(0, 40);

      if (elGlobalLevelsAutoFeatured && rows.length) {
        const best = [...rows].sort(
          (a, b) => (b.like_count | 0) - (b.dislike_count | 0) - ((a.like_count | 0) - (a.dislike_count | 0))
        )[0];
        if (best && best.name)
          elGlobalLevelsAutoFeatured.textContent = `Trending highlight: “${String(best.name)}” (${best.like_count | 0} likes). Admin featured tabs still override rotation.`;
      }
      ...
      const sessionUser = currentSupabaseUser;
      for (const gl of rows) {
        ...
        const sub = document.createElement("div");
        sub.className = "sub";
        const pr = gl.profiles && typeof gl.profiles === "object" ? gl.profiles : null;
        const authorName = pr && pr.username ? String(pr.username) : "Unknown";
        ...
        const likes = gl.like_count | 0;
        const dislikes = gl.dislike_count | 0;
        const tagStr = Array.isArray(gl.tags) && gl.tags.length ? ` · tags: ${gl.tags.join(", ")}` : "";
        sub.textContent = `By ${authorName} · Difficulty ${diff} · ${likes}↑ ${dislikes}↓${vState === "SUSPICIOUS" ? " · Flagged" : ""}${tagStr}`;
        ...
        likeBtn.type = "button";
        likeBtn.className = "btn subtle";
        likeBtn.textContent = "Like";
        likeBtn.addEventListener("click", () => void voteGlobalLevel(gl, 1));
        actions.appendChild(likeBtn);

        const dislikeBtn = document.createElement("button");
        dislikeBtn.type = "button";
        dislikeBtn.className = "btn subtle";
        dislikeBtn.textContent = "Dislike";
        dislikeBtn.addEventListener("click", () => void voteGlobalLevel(gl, -1));
        actions.appendChild(dislikeBtn);

        if (sessionUser && gl.author_id === sessionUser.id) {
          const tagBtn = document.createElement("button");
          tagBtn.type = "button";
          tagBtn.className = "btn subtle";
          tagBtn.textContent = "Tags";
          tagBtn.addEventListener("click", () => void promptAndSaveGlobalTags(gl));
  ...
  function startPlay(sourceLevelId, builtinIndex = null, usedPowerupsFromPrevRun = null, dailyChallengeMeta = null, playOptions = null) {
    ...
    showToast(localMpEnabled ? `${localMpPlayers[localMpTurn]} turn. Sabotage activated.` : "Play mode. Sabotage activated.");
    if (sourceLevelId) {
    ...
    if (play) {
      ...
      elRunHint.textContent = cursed ? `${seedText} · this seed feels cursed.` : `${seedText} · sabotage pattern is learnable.`;
  ...
  function mergeMpPlayOptions(base) {
    ...
    if (base && typeof base === "object") {
      if (base.rankedTrialSession) o.rankedTrialSession = base.rankedTrialSession;
      if (typeof base.rankedForcedSabotage === "number") o.rankedForcedSabotage = base.rankedForcedSabotage;
    }
  ...
  function doRestartPlayCore() {
    ...
    const rankedOpt =
      prev && prev.rankedTrialSession
        ? { rankedTrialSession: prev.rankedTrialSession, rankedForcedSabotage: prev.rankedTrialSession.sabotage }
        : null;
    const merged = mergeMpPlayOptions({ ...(featOpt || {}), ...(rankedOpt || {}) });
    play = createPlayState(
  ...
  function createPlayState(sourceLevelId, builtinIndex = null, runAttempts = 1, selectedPowerups = null, dailyChallengeMeta = null, playOptions = null) {
    ...
    const rankedForcedSabotage =
      playOptions && typeof playOptions.rankedForcedSabotage === "number" && Number.isFinite(playOptions.rankedForcedSabotage)
        ? playOptions.rankedForcedSabotage
        : null;
    const rankedTrialSession =
      playOptions && playOptions.rankedTrialSession && typeof playOptions.rankedTrialSession === "object"
        ? playOptions.rankedTrialSession
        : null;
    const tiles = makeRuntimeTiles(runSeed, sourceLevelId, playFeaturedTier, extraSabotage, rankedForcedSabotage);
  ...
  function makeRuntimeTiles(runSeed, sourceLevelId = null, featuredTier = null, extraSabotage = 0, forcedSabotageLevel = null) {
    /** @type {RuntimeTile[][]} */
    const out = [];
    ...
    const ft = typeof featuredTier === "string" ? featuredTier.trim().toLowerCase() : "";
    if (typeof forcedSabotageLevel === "number" && Number.isFinite(forcedSabotageLevel)) {
      sabotageLevelForRun = clamp(Math.round(forcedSabotageLevel), 1, 10);
    } else if (ft === "medium") sabotageLevelForRun = 6;
  ...
  function updatePlay(dt, now) {
    ...
    if (mpSocket && mpRoomId && play && !play.ended) {
      if (now - mpLastEmitMs >= 50) {
        mpLastEmitMs = now;
        mpSocket.emit("mp_pos", { x: play.player.x, y: play.player.y, vx: play.player.vx });
      }
    }
  ...
  function syncEndOverlay(state) {
    ...
    if (elEndOverlayStats) {
      ...
      if (state.outcome === "win" && typeof state._endCoinMult === "number") {
        stats += ` · coin mult ×${state._endCoinMult.toFixed(2)}`;
      }
      elEndOverlayStats.textContent = stats;
    }
    if (elEndNextLevelBtn) {
      if (state.dailyChallenge || state.rankedTrialSession) {
  ...
  function end(state, outcome, reason) {
    ...
    let prevWinStreak = 0;
    const rankedMidWin =
      outcome === "win" && state.rankedTrialSession && (state.rankedTrialSession.step | 0) < 2;
    if (activePlayer) {
      prevWinStreak = typeof activePlayer.stats.winStreak === "number" ? activePlayer.stats.winStreak : 0;
      if (outcome === "win") {
        activePlayer.stats.totalWins++;
        if (!rankedMidWin) activePlayer.stats.winStreak = prevWinStreak + 1;
        if (state.sourceBuiltinIndex != null) {
          const blT = BUILTIN_LEVELS[state.sourceBuiltinIndex];
          if (blT && blT.tier === "hard") activePlayer.stats.hardBuiltinWins = (activePlayer.stats.hardBuiltinWins | 0) + 1;
          if (blT && blT.tier === "medium") activePlayer.stats.mediumBuiltinWins = (activePlayer.stats.mediumBuiltinWins | 0) + 1;
        }
      } else {
    ...
    if (localMpEnabled && activePlayer && outcome === "win") {
      activePlayer.stats.localMpRounds = (activePlayer.stats.localMpRounds | 0) + 1;
    }

    const diff = lastValidation.difficulty || computeDifficulty(countTiles(grid));
    const runMs = Math.max(0, (state.now || 0) - (state.t0 || 0));
    state._endCoinMult = outcome === "win" ? winStreakCoinMultiplier(prevWinStreak) : 1;
    if (outcome === "lose") {
      ...
      if (state.sourceLevelId) {
        ...
        if (isNewPb) flushTimedLevelPbToSave(state.sourceLevelId);
      }
      const bits = [];
      ...
      if (!rankedMidWin && prevWinStreak >= 1) {
        window.setTimeout(() => {
          showToast(`${prevWinStreak + 1} wins in a row — bonus points!`, 2000);
        }, 500);
      }
    ...
    if (outcome === "win" && state.rankedTrialSession) {
      const rt = state.rankedTrialSession;
      rt.aggDeaths = (rt.aggDeaths | 0) + (state.runDeathCount | 0);
      rt.aggTimeMs = (rt.aggTimeMs | 0) + runMs;
      rt.aggAttempts = (rt.aggAttempts | 0) + (state.runAttempts | 0);
      const stepNow = rt.step | 0;
      if (stepNow < 2) {
        const next = rt.levels[stepNow + 1];
        const blN = BUILTIN_LEVELS[next.builtinIndex];
        if (blN) {
          rt.step = stepNow + 1;
          loadFlatIntoGrid(blN.tilesFlat);
          levelTexts = {};
          levelLinks = {};
          pendingLinkSource = null;
          ensureCheckpointInWorldGrid();
          ...
          persist();
          syncProfileUI();
          showToast(`Ranked trial — stage ${(rt.step | 0) + 1}/3`, 2200);
          startPlay(blN.id, next.builtinIndex, null, null, { rankedForcedSabotage: rt.sabotage, rankedTrialSession: rt });
          return;
        }
      } else if (stepNow === 2) {
        const sc = computeRankedTrialScore({
          timeMs: rt.aggTimeMs,
          deaths: rt.aggDeaths,
          attempts: rt.aggAttempts,
          sabotage: rt.sabotage,
        });
        /** @type {Record<string, number>} */
        let keyObj = {};
        try {
          keyObj = JSON.parse(localStorage.getItem("ssb_ranked_trial_best_v1") || "{}");
        } catch {
          keyObj = {};
        }
        const prevB = Number(keyObj[rt.dayKey]);
        const best = Number.isFinite(prevB) ? Math.max(prevB, sc) : sc;
        keyObj[rt.dayKey] = best;
        try {
          localStorage.setItem("ssb_ranked_trial_best_v1", JSON.stringify(keyObj));
        } catch {
          /* ignore */
        }
        state.rankedTrialSession = null;
        state.obsessionNote = `Ranked score ${sc} (best today ${Math.round(best)})`;
        if (activePlayer) {
          activePlayer.stats.rankedTrialsCompleted = (activePlayer.stats.rankedTrialsCompleted | 0) + 1;
          const coinBonus = Math.max(5, Math.round(35 * winStreakCoinMultiplier(prevWinStreak)));
          setLocalCoins(getLocalCoins() + coinBonus);
          showToast(`Ranked trial complete! +${coinBonus} local coins`, 2800);
        }
      }
    } else if (outcome === "lose" && state.rankedTrialSession) {
      state.rankedTrialSession = null;
      showToast("Ranked trial ended.", 2000);
    }
            ...
            if (needsCoinAward) {
              ...
              if (aErr) {
                ...
                if (typeof awarded === "number" && awarded > 0) {
                  const sm = winStreakCoinMultiplier(prevWinStreak);
                  const extra = Math.max(0, Math.min(120, Math.floor(awarded * (sm - 1))));
                  if (extra > 0) {
                    try {
                      const { error: exErr } = await sb.rpc("ssb_grant_streak_bonus_coins", { p_extra: extra });
                      if (exErr) logSupabaseError("ssb_grant_streak_bonus_coins(daily)", exErr, { p_extra: extra });
                      else showToast(`Streak bonus: +${extra} coins`, 2000);
                    } catch (e) {
                      logSupabaseError("ssb_grant_streak_bonus_coins(daily) exception", e, {});
                    }
                  }
                  if (activePlayer) activePlayer.stats.dailyClears = (activePlayer.stats.dailyClears | 0) + 1;
                  await fetchUserCoinsAndShopState(sb, u);
                  syncShopUI();
    ...
    if (!currentSupabaseUser && activePlayer && outcome === "win" && !isBlockedAccountNow()) {
      ...
      if (newDailyLocal) activePlayer.stats.dailyClears = (activePlayer.stats.dailyClears | 0) + 1;
      if (bonus > 0) {
        const coinMult = winStreakCoinMultiplier(prevWinStreak);
        const add = Math.max(0, Math.floor(bonus * coinMult));
        setLocalCoins(getLocalCoins() + add);
        syncShopUI();
        showToast(`+${add} coins (×${coinMult.toFixed(2)} streak)`, 2000);
      }
    }
    ...
    if (outcome === "win" && isSupabaseConfigured()) {
      ...
      void (async () => {
        await pushGlobalScore(earned, preMeta, outcome === "win" ? prevWinStreak : 0);
  ...
  function formatAuthError(err) {
    ...
    if (rawLower.includes("invalid api key") || rawLower.includes("invalid jwt")) {
      return "Invalid Supabase key/config. Re-check SUPABASE_URL and SUPABASE_ANON_KEY.";
    }
  ...
  function initMultiplayer() {
    ...
    if (elMpJoinBtn && elMpJoinInput) {
      ...
      elMpJoinBtn.addEventListener("click", () => doJoin());
