...
const extraCorsOrigins = parseList(process.env.CORS_ORIGIN, []);
...
const config = {
  ...
  /**
   * Comma-separated origins. Default list includes *.vercel.app (Vercel game) plus common backend hosts for multiplayer/API.
   * Override with CORS_ORIGIN for your exact production domains.
   * Example: https://my-game.vercel.app,*.vercel.app
   */
  corsOrigins: mergeUnique(defaultCorsOrigins, extraCorsOrigins),
  ...
  game: {
    ...
    roundsPerMatch: Math.max(1, intEnv("ROUNDS_PER_MATCH", 5)),
