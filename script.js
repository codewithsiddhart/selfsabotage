(() => {
  ...
  function openStartModal() {
    if (deviceMode === "mobile" && window.MobileUI) {
      window.MobileUI.showHome();
      return;
    }
  ...
  // Undo/redo history for build-mode grid edits. Snapshots are cheap (COLS*ROWS small ints),
  // so we just keep flat-array copies rather than diffing.
  const UNDO_LIMIT = 50;
  let undoStack = [];
  let redoStack = [];
  function pushUndoSnapshot() {
    undoStack.push(flattenGrid(grid));
    if (undoStack.length > UNDO_LIMIT) undoStack.shift();
    redoStack.length = 0;
  }
  function undo() {
    if (undoStack.length === 0) return false;
    redoStack.push(flattenGrid(grid));
    const snap = undoStack.pop();
    inflateGrid(grid, snap, COLS, ROWS);
    scheduleValidate();
  ...
  // ---------- Mobile build-canvas gesture state ----------
  // Desktop behavior is untouched (single click place/erase). On mobile, the canvas additionally
  // supports: single-finger drag to paint/erase continuously, two-finger pinch to zoom + pan, and
  // double-tap to toggle zoom. All of this is layered on top of the same placeTile/eraseTileAt/
  // canPlaceTile primitives desktop uses, so there is one source of truth for grid edits.
  /** @type {Map<number, {x:number, y:number}>} */
  const activeCanvasPointers = new Map();
  let paintPointerId = null;
  let paintErase = false;
  let lastPaintCell = null;
  let pinchGesture = null; // { startDist, startScale, startMidX, startMidY, startTx, startTy }
  let lastTapTime = 0;
  let lastTapX = 0;
  let lastTapY = 0;
  const buildZoom = { scale: 1, tx: 0, ty: 0 };
  const BUILD_ZOOM_MIN = 1;
  const BUILD_ZOOM_MAX = 3;

  function clampBuildZoom() {
    buildZoom.scale = clamp(buildZoom.scale, BUILD_ZOOM_MIN, BUILD_ZOOM_MAX);
    const vp = document.getElementById("buildZoomViewport");
    const w = vp ? vp.clientWidth : canvas.clientWidth;
    const h = vp ? vp.clientHeight : canvas.clientHeight;
    const minTx = w * (1 - buildZoom.scale);
    const minTy = h * (1 - buildZoom.scale);
    buildZoom.tx = clamp(buildZoom.tx, minTx, 0);
    buildZoom.ty = clamp(buildZoom.ty, minTy, 0);
  }

  function applyBuildZoom() {
    clampBuildZoom();
    canvas.style.transform = `translate(${buildZoom.tx}px, ${buildZoom.ty}px) scale(${buildZoom.scale})`;
    canvas.style.transformOrigin = "0 0";
  }

  function resetBuildZoom() {
    buildZoom.scale = 1;
    buildZoom.tx = 0;
    buildZoom.ty = 0;
  ...
  function toggleDoubleTapZoom(clientX, clientY) {
    ...
    buildZoom.tx = px - px * targetScale;
    buildZoom.ty = py - py * targetScale;
    buildZoom.scale = targetScale;
    applyBuildZoom();
  }
  ...
  canvas.addEventListener("pointermove", (e) => {
    if (deviceMode === "mobile" && activeCanvasPointers.has(e.pointerId)) {
      activeCanvasPointers.set(e.pointerId, { x: e.clientX, y: e.clientY });
    }

    // Two-finger pinch/pan (mobile build only)
    if (deviceMode === "mobile" && mode === "build" && pinchGesture && activeCanvasPointers.size >= 2) {
      const pts = Array.from(activeCanvasPointers.values()).slice(0, 2);
      const dist = distanceSq(pts[0].x, pts[0].y, pts[1].x, pts[1].y) ** 0.5;
      const midX = (pts[0].x + pts[1].x) / 2;
      const midY = (pts[0].y + pts[1].y) / 2;
      const scale = clamp(
        pinchGesture.startScale * (dist / Math.max(1, pinchGesture.startDist)),
        BUILD_ZOOM_MIN,
        BUILD_ZOOM_MAX
      );
      buildZoom.scale = scale;
      buildZoom.tx = pinchGesture.startTx + (midX - pinchGesture.startMidX);
      buildZoom.ty = pinchGesture.startTy + (midY - pinchGesture.startMidY);
      applyBuildZoom();
      return;
    }

    // Single-finger drag paint/erase (mobile build only)
    if (deviceMode === "mobile" && mode === "build" && paintPointerId === e.pointerId) {
      const { gx, gy } = canvasToGrid(e);
      if (inBounds(gx, gy) && (!lastPaintCell || lastPaintCell.gx !== gx || lastPaintCell.gy !== gy)) {
        lastPaintCell = { gx, gy };
        if (paintErase) eraseTileAt(gx, gy);
        else {
          const check = canPlaceTile(gx, gy, selectedTile);
          if (check.ok) placeTile(gx, gy, selectedTile);
        }
      }
      return;
    }
  ...
  canvas.addEventListener("pointerdown", (e) => {
    ...
    if (deviceMode === "mobile") {
      ...
      if (activeCanvasPointers.size >= 2) {
        // A second finger landed: switch to pinch/pan, cancel any in-progress paint.
        paintPointerId = null;
        lastPaintCell = null;
        const pts = Array.from(activeCanvasPointers.values()).slice(0, 2);
        pinchGesture = {
          startDist: Math.max(1, (distanceSq(pts[0].x, pts[0].y, pts[1].x, pts[1].y)) ** 0.5),
          startScale: buildZoom.scale,
          startMidX: (pts[0].x + pts[1].x) / 2,
          startMidY: (pts[0].y + pts[1].y) / 2,
          startTx: buildZoom.tx,
          startTy: buildZoom.ty,
        };
        return;
      }
      ...
      const now = performance.now();
      const isDoubleTap =
        now - lastTapTime < 300 && Math.abs(e.clientX - lastTapX) < 26 && Math.abs(e.clientY - lastTapY) < 26;
      lastTapTime = now;
      lastTapX = e.clientX;
      lastTapY = e.clientY;
      if (isDoubleTap) {
      ...
      paintErase = selectedTile === Tile.empty;
      lastPaintCell = { gx, gy };
      if (paintErase) eraseTileAt(gx, gy);
      else {
        const check = canPlaceTile(gx, gy, selectedTile);
        if (!check.ok) showToast(check.reason || "Can't place here.");
        else placeTile(gx, gy, selectedTile);
      }
      return;
    }
  ...
  function endCanvasPointer(e) {
    ...
    if (activeCanvasPointers.size < 2) pinchGesture = null;
    try {
  ...
  canvas.addEventListener("pointerup", (e) => {
    if (deviceMode === "mobile") endCanvasPointer(e);
  });
  canvas.addEventListener("pointercancel", (e) => {
    if (deviceMode === "mobile") endCanvasPointer(e);
  });

  function eraseTileAt(gx, gy) {
  ...
  /** @param {number} index Index into BUILTIN_LEVELS */
  function launchBuiltinLevel(index) {
  ...
  /**
   * @param {boolean} [resetSource] Pass true when the grid underneath has just been replaced
   * with something other than the run's original source (e.g. Clear Grid, or Load-ing a
   * different saved level while still in Play mode). Without this, the new run would keep
   * inheriting the old sourceLevelId/sourceBuiltinIndex — wrongly carrying built-in-only
   * mechanics (like the hammer, timer limit, or powerup consumption) onto a level that isn't
   * actually that built-in level anymore.
   */
  function restartPlay(resetSource = false) {
    if (mode !== "play") return;
    if (mpSession.active && (mpSession.phase === "mpPlayOpponent" || mpSession.phase === "mpRound3")) {
    ...
    hideEndOverlay();
    const prev = play;
    const sourceLevelId = resetSource ? null : prev ? prev.sourceLevelId : null;
    const sourceBuiltinIndex = resetSource ? null : prev ? prev.sourceBuiltinIndex : null;
  ...
  function updatePlay(dt, now) {
    ...
    // Action hotkeys (restart works both mid-run and after the run has ended)
  ...
  function updateRuntimeTiles(state, dt) {
    ...
    for (let y = 0; y < ROWS; y++) {
      for (let x = 0; x < COLS; x++) {
        ...
        // "oneStep" already flips solid=false immediately in onLand(), but "delayed" and
        // "flickerThenBreak" only arm tile.breakTimer there and rely on this countdown to
        // actually break the platform — so solid must be cleared here once it hits 0, or
        // those two sabotage types silently never break.
  ...
  function onLand(state, gx, gy) {
    ...
    if (tile.type === Tile.jumppad && tile.padCooldownMs === 0) {
      ...
      if (sab.type === "delayed") {
        ...
        // Capture this run's identity now — if the player restarts (or leaves) before the
        // timeout fires, `play` will point at a brand-new (not-ended) run and this stale
        // callback must not reach into it.
        const scheduledRunSeed = state.runSeed;
        setTimeout(() => {
          if (!play || play.ended || play.runSeed !== scheduledRunSeed) return;
  ...
  function connectMultiplayerSocket() {
    ...
    const socket = io({ query: { name }, transports: ["websocket", "polling"] });
    ...
    socket.on("connect_error", () => {
      showToast("Could not connect to multiplayer server. Use the same host as the game (e.g. your ngrok URL).", 3800);
      resetMultiplayerClientUi();
    });
  ...
  function frame(now) {
    ...
    if (window.MobileUI && window.MobileUI.sync) window.MobileUI.sync();
  ...
  if (!deviceMode && elDeviceModal) {
    ...
    if (deviceMode === "mobile" && window.MobileUI) window.MobileUI.showHome();
  }
  ...
  // =========================================================
  // Public API for the mobile UI shell (mobile.js)
  // ---------------------------------------------------------
  // This is the ONLY bridge between the engine and the mobile screens. Everything here is a thin
  // pass-through to functions/state already used by the desktop UI above — no gameplay logic is
  // duplicated. mobile.js is free to build its own DOM/CSS on top of this; it never reaches into
  ...
  window.SSB = {
    ...
    // live state getters (always read the current value, never a stale snapshot)
    getMode: () => mode,
    ...
    getPendingPowerups: () => pendingPowerupsForRun,
    getMpSession: () => mpSession,
    getBuiltinLevels: () => BUILTIN_LEVELS,
    getBuiltinLevelStats: () => builtinLevelStats,
    ...
    createPlayer,
    setActivePlayer,
    togglePendingPowerup: (key) => {
      if (!activePlayer || !activePlayer.powerups) return;
      pendingPowerupsForRun[key] = !pendingPowerupsForRun[key];
      syncPowerupUI();
    },
    showToast,
